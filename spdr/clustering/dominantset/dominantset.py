...
from .evaluation import get_acp, get_mr, get_most_partecipating, get_hungarian, get_ari
...
class DominantSetClustering:
    def __init__(self, feature_vectors, speaker_ids, metric,
                 ...
                 dominant_search=False, similarity_list=None, apply_similarity_mode="full"):
        """
        :param metric:
            'cosine'
            'euclidean'
        ...
        self.apply_similarity_mode = apply_similarity_mode
    ...
    def get_adj_matrix(self):
        if self.metric == 'euclidean':
            ...
            dist_mat = distance.pdist(self.feature_vectors, metric=self.metric)
            dist_mat = distance.squareform(dist_mat)
            normalized_distmat = preprocessing.normalize(dist_mat)
            # dist_mat = pw.cosine_similarity(self.feature_vectors)
            dist_mat = np.arccos(normalized_distmat)
        ...
        if self.similarity_list is not None:
            self.apply_similarity_list(self.adj_matrix)
    ...
    def evaluate(self):
        ...
        mr = get_mr(labels=v, ground_truth=self.speaker_ids)

        # Cluster Purity
        acp = get_acp(labels=self.ds_result, ground_truth=self.speaker_ids)

        # Adjusted Random Index
        randi = get_ari(labels=self.ds_result, ground_truth=self.speaker_ids)

        return mr, randi, acp

    # list with known clusters. each cluster is represented by a list with indexes to the corresponding elements
    def apply_similarity_list(self, dist_mat):
        similarity = np.max(dist_mat)

        for ele in self.similarity_list:
            if isinstance(ele, list):
                if self.apply_similarity_mode in ["full", "similar"]:
                    for pair in itertools.combinations(ele, 2):
                        dist_mat[pair[0]][pair[1]] = similarity
                        dist_mat[pair[1]][pair[0]] = similarity
            elif ele + 1 < dist_mat.shape[0]:
                if self.apply_similarity_mode in ["full", "different"]:
                    dist_mat[(ele, ele + 1)] = 0.0
                    dist_mat[(ele + 1, ele)] = 0.0
